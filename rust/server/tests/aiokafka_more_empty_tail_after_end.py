...
async def empty_seek_to_end_after_first_record_matrix(bootstrap: str) -> None:
    consumer = AIOKafkaConsumer(bootstrap_servers=bootstrap)
    await consumer.start()
    try:
        topic_partition = TopicPartition("python.empty-tail-after-end", 0)
        ...
        try:
            await producer.send_and_wait(
                "python.empty-tail-after-end", b"first", partition=0
            )
        finally:
            await producer.stop()
        ...
        try:
            await producer.send_and_wait(
                "python.empty-tail-after-end", b"second", partition=0
            )
        finally:
            await producer.stop()
...
async def main() -> int:
    await empty_seek_to_end_after_first_record_matrix(sys.argv[1])
    return 0
