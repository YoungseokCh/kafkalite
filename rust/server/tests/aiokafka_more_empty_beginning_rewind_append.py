...
async def empty_seek_to_beginning_then_rewind_and_append_matrix(bootstrap: str) -> None:
    consumer = AIOKafkaConsumer(bootstrap_servers=bootstrap)
    await consumer.start()
    try:
        topic_partition = TopicPartition("python.empty-beginning-rewind-append", 0)
        ...
        try:
            await producer.send_and_wait(
                "python.empty-beginning-rewind-append", b"first", partition=0
            )
        finally:
            await producer.stop()
        ...
        try:
            await producer.send_and_wait(
                "python.empty-beginning-rewind-append", b"second", partition=0
            )
        finally:
            await producer.stop()
...
async def main() -> int:
    await empty_seek_to_beginning_then_rewind_and_append_matrix(sys.argv[1])
    return 0
