...
async def empty_seek_to_end_then_seek_absolute_matrix(bootstrap: str) -> None:
    consumer = AIOKafkaConsumer(bootstrap_servers=bootstrap)
    await consumer.start()
    try:
        topic_partition = TopicPartition("python.empty-tail-seek", 0)
        ...
        try:
            await producer.send_and_wait(
                "python.empty-tail-seek", b"first", partition=0
            )
            await producer.send_and_wait(
                "python.empty-tail-seek", b"second", partition=0
            )
        finally:
            await producer.stop()
...
async def main() -> int:
    await empty_seek_to_end_then_seek_absolute_matrix(sys.argv[1])
    return 0
